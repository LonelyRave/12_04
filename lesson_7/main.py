...
def get_file_name_length(file_path):
    """Возвращает длину имени файла."""
    return len(os.path.basename(file_path))


def count_files_and_folders(path, max_count=sys.maxsize, serialized_results=None):
    """
    Подсчитывает количество файлов и папок по заданному пути или по всей системе для macOS.
    При прерывании (KeyboardInterrupt) сохраняет результаты в файл serialized_results для продолжения работы.
    Возвращает кортеж с количеством папок, файлов и информацией о самом большом/маленьком файле, а также о самом
    длинном/коротком имени файла.
    """
    try:
        ...
        # Сохраняем результаты в файл для продолжения работы
